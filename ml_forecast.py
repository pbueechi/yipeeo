...
class nc2table:
    ...
    def merge_tabs(self,temp_res):
        if self.farm:
            path = f'Data/{temp_res}/{self.farm}_{self.crop}*.csv'
        else:
            path = f'Data/{temp_res}/{self.country}_{self.crop}*.csv'
        files = glob(path)
        ...
        if self.farm:
            csv_fin.to_csv(f'Data/{temp_res}/{self.farm}_{self.crop}_all.csv')
        else:
            csv_fin.to_csv(f'Data/{temp_res}/{self.country}_{self.crop}_all.csv')
...
def get_default_regressions(regression_names):
    """
...
def merge_previous_months(file):
...
def runforecast(crop, country, lead_time,predictors, model='RF'):
    file = pd.read_csv(f'Data/2W/{country}_{crop}_{predictors}.csv', index_col=0)
    ...
    predictor_file = merge_previous_months(file.loc[:,used_predictors])
    ...
    pipe = Pipeline([('scalar', StandardScaler()), ('clf', get_default_regressions(model))])
...
def write_results():
    ...
    for lead_time in lead_times:
        for pred in cols[1:]:
            csv_file.loc[lead_time,pred] = runforecast(country='cz',crop=crops[1], lead_time=lead_time, predictors=pred)
...
def plot_res():
    ...
    print(final)
    # print(file.lead_time)
    # print(file)
    # print(list(file.s1[8])[0])
...
if __name__ == '__main__':
    ...
    #     runforecast(country='cz',crop=crops[1], lead_time=lead_time)
