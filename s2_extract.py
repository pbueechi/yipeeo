...
class s2:
    ...
    def extract_s2(self, items, core=None, new=True):
        """
        ...
        #     signed_item = planetary_computer.sign(item)
            ...
            #     fields = self.fields.to_crs(src.crs)
            #
            ...
            #                                                                       np.nanstd(out_image)]
            #                 else:
            #                     fields_data.loc[:, polygon.field_id.values[0]] = [np.nanmedian(out_image),
            #                                                                       np.nanstd(out_image)]
            #         except:
            #             fields_data.loc[:, polygon.field_id.values[0]] = [np.nan, np.nan]
    ...
    def find_s2_items(self):
        ...
        #ToDo if file does not contain c_year, adjust year_min and max to 2016 and 2022
        year_min, year_max = np.nanmin(self.fields.c_year.values), np.nanmax(self.fields.c_year.values)
        ...
        fields = self.fields.drop_duplicates(subset='field_id')
    ...
    def run_extraction(self, n_cores=4, new=False):
        ...
        search_s2 = self.find_s2_items()[:1]
        print(search_s2)
        with ProcessPoolExecutor(max_workers=mp.cpu_count()) as pool:
            for i in range(n_cores):
                ...
                # self.extract_s2(items=this_search_s2, core=1, new=new)
                pool.submit(self.extract_s2, items=this_search_s2, core=i, new=new)
                print('done')
